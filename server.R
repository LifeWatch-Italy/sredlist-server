...
# Load Map countries
coo_raw<-read_sf("Species/Map countries/Red_List_countries_msSimplif_coo_0.001.gpkg") ; names(coo_raw)<-c("SIS_name0", "SIS_name1", "lookup", "lookup_SIS0", "geometry") ; coo_raw$lookup_SIS0[coo_raw$SIS_name0=="Namibia"]<-"NA" # Used to map COO; Namibia should be "NA" and not NA
...
Area_Remove_PanEU <- st_read("Species/Map countries/Area_to_remove_from_PanEU.gpkg") # A small part of South European Russia has to be removed for PanEuropean map
distCountries_mapping <- st_read("Species/Map countries/Red_List_countries_msSimplif0.05_MOLL.gpkg")  ; st_crs(distCountries_mapping)<-CRSMOLL # Use to create maps in GBIF step 3
distCountries_WGS <- st_read("Species/Map countries/Red_List_countries_msSimplif0.05_WGS.gpkg") # Used to filter GBIF data in GBIF step 2
distCountries<-st_read("Species/Map countries/Red_List_countries_msSimplif0.005_MOLL.gpkg") ; st_crs(distCountries)<-CRSMOLL # Used to map countries in the background
