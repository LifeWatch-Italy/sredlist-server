...
library(adehabitatHR) ; library(smoothr) ; library(alphahull) ; library(nngeo)
...
# Load Map countries
coo_raw<-read_sf("Species/Map countries/Red_List_countries_msSimplif_coo_0.001.gpkg") ; coo_raw$lookup_SIS0[coo_raw$SIS_name0=="Namibia"]<-"NA" ; st_geometry(coo_raw) <- "geometry" # Used to map COO; Namibia should be "NA" and not NA
...
distCountries_NRL <- st_read("Species/Map countries/Countries_NRL_landmarCombined_msSimplif_0.05.gpkg") ;  st_geometry(distCountries_NRL) <- "geometry" # Use to crop countries in 1a and 1b for NRL
Area_Remove_PanEU <- st_read("Species/Map countries/Area_to_remove_from_PanEU.gpkg") ; st_geometry(Area_Remove_PanEU) <- "geometry" # A small part of South European Russia has to be removed for PanEuropean map
distCountries_mapping <- st_read("Species/Map countries/Red_List_countries_msSimplif0.05_MOLL.gpkg") ; st_geometry(distCountries_mapping) <- "geometry"  ; st_crs(distCountries_mapping)<-CRSMOLL # Use to create maps in GBIF step 3
distCountries_WGS <- st_read("Species/Map countries/Red_List_countries_msSimplif0.05_WGS.gpkg") ; st_geometry(distCountries_WGS) <- "geometry" # Used to filter GBIF data in GBIF step 2
distCountries<-st_read("Species/Map countries/Red_List_countries_msSimplif0.005_MOLL.gpkg") ; st_geometry(distCountries) <- "geometry" ; st_crs(distCountries)<-CRSMOLL # Used to map countries in the background
