...
if("Gbif_Extract_Habitat" %in% outlog$Parameter){
  readPNG(paste0("resources/AOH_stored/", sub(" ", "_", scientific_name), "/Plots/gbifHabExtract.png")) %>% rasterGrob() %>% grid.arrange()
}
```
...
The calculated Area of Habitat has a value of `r pretty_num(ceiling(Storage_SP$AOHkm2_saved))` km2. 
...
When rescaled at a 2x2km grid, this provides a value of `r pretty_num(round(Storage_SP$aoo_km2))` km2; this value can be used (if you trust the map) as an upper bound of area of occupancy if you consider likely that all the mapped suitable habitat is occupied by the species. If you know part of it is not occupied, you might want to reduce this estimate. `r if(Storage_SP$AOH_type=="Large"){"Note that, as your species has a large range, the platform used a simplified version of land-cover which may lead the upper bound of AOO to be slightly overestimated."}`.
