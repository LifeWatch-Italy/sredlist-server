...
<br>
<center>
![](`r paste0(outDir, '/Plots/plot_data.png')`){width=100%}
</center>
...
`r paste0("You kept only occurrence records with basis of records: ", outlog$Value[outlog$Parameter=="Gbif_BasisOfRecord"] %>% sub("bor_human", "Human observation", .) %>% sub("bor_preserved", "Preserved specimen", .) %>% sub("bor_fossil", "Fossil specimen", .) %>% sub("bor_machine", "Machine observation", .) %>% sub("bor_living", "Living specimen", .) %>% sub("bor_other", "Other", .) %>% sub(",", ", ", .), ".")` 
...
Tab <- Storage_SP$flags[, !names(Storage_SP$flags) %in% c("species", "species_download", "coordinateUncertaintyInMeters", "individualCount", "family", "taxonRank", "genericName", "specificEpithet", "institutionCode", "datasetName", ".val", ".equ", ".zer", ".cap", ".cen", ".gbf", ".inst", ".year", ".uncertainty", ".limits", "Only_for_syn", "PopText", "Coords_merged", "unique", "Lon_jitt", "Lat_jitt", "_leaflet_id", "ID")]
...
Col_order <- c("Longitude", "Latitude", "ID", "Source_type", "basisOfRecord", "Invalid_reason") %>% .[. %in% names(Tab)]
...
`r if("TableNonGeo" %in% names(Storage_SP)){"You also checked the table of non-georeferenced occurrence records from GBIF: "}`
```{r leaflet_nongeo, fig.align = "center", out.width = '100%'}
if("TableNonGeo" %in% names(Storage_SP)){
  DT::datatable(Storage_SP$TableNonGeo, 
              rownames=F,
              filter="top",
              escape=F,
              options=list(
    headerCallback = DT::JS(# Needed to reduce column name font
      ...
      "  $(thead).css('font-size', '0.75em');",
      "}"))
    ) %>% DT::formatStyle(columns = c(1:ncol(Storage_SP$TableNonGeo)), fontSize = '75%')
}
...
<center>
![](`r if("Gbif_Extract_Elevation" %in% outlog$Parameter){paste0(outDir, '/Plots/gbifElevExtract.png')}`){width=100%}
</center>

`r if("Gbif_Extract_Bathymetry" %in% outlog$Parameter){"The bathymetry of occurrence records was extracted: "}`

<center>
![](`r if("Gbif_Extract_Bathymetry" %in% outlog$Parameter){paste0(outDir, '/Plots/gbifElevExtract_bathy.png')}`){width=100%}
</center>
...
PRINT_starting<-outlog$Value[outlog$Parameter=="Mapping_Start"] %>% plyr::revalue(., c(
  ...
  "coastal"="draw coastline where the species occurs",
  "vents"="map hydrothermal vents where the species occurs",
...
ELEV_text <- paste0(
  ifelse((ELEV[1]=="0" & ELEV[2]=="9000"), "", paste0("You asked to restrict the polygon to area that is between ", ELEV[1], " and ", ELEV[2], "m of elevation. ")),
  ifelse((ELEV[3]=="-11000" & ELEV[4]=="0"), "", paste0("You asked to restrict the polygon to area that is between ", ELEV[4], " and ", ELEV[3], "m of bathymetry. "))
  )
...
`r if(outlog$Value[outlog$Parameter=="Mapping_Altitude"] != "0, 9000, -11000, 0"){ELEV_text}`  
...
<br>
<center>
![](`r paste0(outDir, '/Plots/gbifStep3.png')`){width=100%}
</center>
...
<center>
![](`r if("Gbif_EditPoly" %in% outlog$Parameter){paste0(outDir, '/Plots/plot_manually_edited.png')}`){width=100%}
</center>
