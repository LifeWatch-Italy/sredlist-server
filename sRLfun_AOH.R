...
sRL_CompareGenerationLength <- function(scientific_name, GL_SIS_raw, GL_file){
  ...
  # By genus
  genuses_group <- order %>% subset(., family_name==order$family_name[order$scientific_name==scientific_name]) %>% dplyr::group_by(genus_name) %>% dplyr::summarise(N=n(), Mean_GL=mean(GL_valueQTT, na.rm=T), n_values=length(GL_valueQTT[is.na(GL_valueQTT)==F]))
  genuses <- order %>% 
    subset(., family_name==order$family_name[order$scientific_name==scientific_name] & is.na(GL_valueQTT)==F) %>% 
