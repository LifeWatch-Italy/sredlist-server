...
sRL_ChargeBathyRaster<-function(){alt_raw<-rast(sub("Elevation_MollweidePlatform", "Bathymetry_MollweidePlatform", config$alt_raster_path)) ; crs(alt_raw)<-CRSMOLL ; names(alt_raw)[1]<-"Elevation_reprojMollweide3" ; return(alt_raw)}  # I ensure the CRS is correctly assigned (it was saved as a CRSMOLL raster)}
